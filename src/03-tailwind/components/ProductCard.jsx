  ...
  return (
    ...
    >
      <section className="flex flex-col justify-between gap-8 sm:gap-10 md:gap-15 md:flex-row">
        <div className="bg-blue-50 max-w-full w-full rounded-sm flex justify-center items-end relative md:max-w-[400px]">
          {product.tag &&
            <span
              aria-label={`Tag: ${product.tag}`}
              className={`absolute top-2 left-2 w-fit py-[1px] px-[6px] font-[.9rem] font-medium rounded-sm text-white ${product.tag === "Novo" ? "bg-blue-700" : "bg-red-300"}`}
            >
              {product.tag}
            </span>
          }
      ...
      <section>
        ...
        <section className="flex flex-col">
          ...
          {product.reviews.length > 0 && (
            <button className="border-2 text-medium border-transparent rounded-sm bg-blue-700 text-white py-1 px-8 cursor-pointer
              hover:not-[:active]:border-blue-700
              dark:hover:not-[:active]:border-white
              hover:text-blue-700 dark:hover:text-white
              hover:not-[:active]:bg-transparent
              active:bg-transparent active:text-blue-700 active:scale-90
              transition-all duration-250 mt-6 self-center">
              Ver mais avaliações
            </button>
          )}
        </section>
